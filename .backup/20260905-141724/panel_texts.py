...
@router.callback_query(F.data == "story_add")
async def cb_story_add(cb):
    refs = load_story_refs(config.STORIES_FILE)
    rows = [
        "➕ <b>Добавить story</b>",
        f"В пуле: <b>{len(refs)}</b>",
        "",
        "Пришли ссылку (можно несколько строк):",
        "<code>https://t.me/channel/s/1</code>",
        "<code>t.me/channel/s/1</code>",
        "<code>channel|42</code>",
        "",
        "Дубликаты не добавляются повторно.",
        "Рассылка подхватит за ~30 сек без перезапуска.",
    ]
    reset_state(cb.from_user.id)
    state(cb.from_user.id)["wait"] = "story_add"
    await cb.message.answer("\n".join(rows), reply_markup=kb_back_texts())
    await cb.answer()


@router.callback_query(F.data == "story_remove")
async def cb_story_remove(cb):
    refs = load_story_refs(config.STORIES_FILE)
    rows = [
        "🗑 <b>Убрать story из пула</b>",
        f"В пуле: <b>{len(refs)}</b>",
        "",
        "Пришли ссылку или <code>peer|id</code> для удаления.",
    ]
    reset_state(cb.from_user.id)
    state(cb.from_user.id)["wait"] = "story_remove"
    await cb.message.answer("\n".join(rows), reply_markup=kb_back_texts())
    await cb.answer()
...
@router.message(waiting("href_domain", "href_bot", "story_add", "story_remove", "story_toggle"))
async def texts_input(message):
    s = state(message.from_user.id)
    wait = s.get("wait")
    text = (message.text or message.caption or "").strip()

    if wait in ("story_add", "story_toggle") and text:
        ...
        for line in lines:
            try:
                ...
                bad += 1
                await message.answer(f"❌ Ошибка записи stories.txt: <code>{esc(e)}</code>")
                return
            if status == "invalid":
                bad += 1
            elif status == "exists":
        ...
        rows = [
            f"✅ добавлено: <b>{added}</b> | уже было: <b>{exists}</b> | ⚠️ неверный формат: <b>{bad}</b>",
            f"В пуле: <b>{len(load_story_refs(config.STORIES_FILE))}</b>",
        ]
        if added_labels:
    ...
    if wait == "story_remove" and text:
        lines = [normalize_story_input(ln) for ln in text.splitlines() if ln.strip()]
        removed = missing = bad = 0
        removed_labels = []
        total = len(load_story_refs(config.STORIES_FILE))
        for line in lines:
            try:
                status, total, label = remove_story_ref(line, config.STORIES_FILE)
            except Exception as e:
        ...
        rows = [
            f"🗑 удалено: <b>{removed}</b> | не найдено: <b>{missing}</b> | ⚠️ формат: <b>{bad}</b>",
            f"В пуле: <b>{total}</b>",
        ]
        if removed_labels:
