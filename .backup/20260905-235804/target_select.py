...
async def collect_targets(client, rng, max_offline_days: int) -> list[Target]:
    ...
    try:
        ...
        log.warning(f"collect_targets: контакты — {type(e).__name__}: {e}")
    ...
    try:
        ...
        log.warning(f"collect_targets: диалоги — {type(e).__name__}: {e}")

    if not contacts_ok and not groups_ok and not targets:
        log.warning("collect_targets: оба запроса упали, целей нет")

    users = [t for t in targets if t.kind == "user"]
    groups = [t for t in targets if t.kind == "group"]
    users.sort(key=lambda t: getattr(t.entity, "id", 0))
    rng.shuffle(groups)
    return users + groups
