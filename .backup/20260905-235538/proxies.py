...
class ProxyPool:
    ...
    def acquire(self):
        ...
        if not free:
            return None
        under_cap = [p for p in free if p["in_use"] < MAX_IN_USE_PER_PROXY]
        pool = under_cap if under_cap else free
        p = min(pool, key=lambda x: x["in_use"])
        p["in_use"] += 1
        return p
    ...
    def reset_stuck(self):
        ...
        for p in self.proxies:
            if p["in_use"] > MAX_IN_USE_PER_PROXY * 2:
                p["in_use"] = max(0, p["in_use"] - MAX_IN_USE_PER_PROXY)
            elif p["in_use"] < 0:
                p["in_use"] = 0
