    ...
    async def send_loop(self, client, sid, path, targets, rng, story_cache, session_ready):
        ...
        while True:
            if time.time() - start >= stint:
                log.info(
                    f"{sid} | ♻️ смена слота (~{stint}с), реально {sent_local} "
                    f"(ЛС {sent_contacts_local}/{len(users)}), тень {shadow_local}"
                )
                return "rotate"
            ...
            if target is None:
                ...
                prefer_user = True
                log.info(
                    f"{sid} | 🔁 круг: все {len(users)} ЛС + {len(groups)} групп пройдены | "
