...
def move_session_to_bad(path):
    """Перенос мёртвой сессии в sessions_bad (не удаляем навсегда)."""
    moved = False
    try:
...
class Spammer:
    def __init__(self):
        ...
        self.sessions_to_bad = 0
        self.sessions_no_targets = 0
        self.session_cursors: dict[str, tuple[int, int, bool]] = {}
    ...
    def _cursor_key(self, path: str) -> str:
        return os.path.basename(path)

    def _save_session_cursor(
        self, sess_file: str, contact_idx: int, group_idx: int, prefer_user: bool
    ):
        self.session_cursors[sess_file] = (contact_idx, group_idx, prefer_user)

    def _load_session_cursor(self, sess_file: str, n_users: int, n_groups: int) -> tuple[int, int, bool]:
        ci, gi, pu = self.session_cursors.get(sess_file, (0, 0, True))
        ci = min(max(0, ci), n_users)
        gi = min(max(0, gi), n_groups)
        if ci >= n_users and gi >= n_groups:
            return 0, 0, True
        return ci, gi, pu

    @staticmethod
    def _stint_hard_sec() -> float:
        return config.REFRESH_INTERVAL * config.STINT_HARD_MULTIPLIER
    ...
    async def write_stats_loop(self):
        while not self.stop.is_set():
            ...
            data = {
                ...
                "sessions_to_bad": self.sessions_to_bad,
                "sessions_no_targets": self.sessions_no_targets,
            }
            try:
    ...
    async def log_stats_loop(self):
        while not self.stop.is_set():
            ...
            log.info(
                ...
                f"ЛС: {self.total_contacts} | группы: {self.total_groups}"
            )
        ...
        for story in available:
            ...
            try:
                ...
                if await self._story_exists_for_account(client, input_peer, story.story_id):
                    ...
                    log.warning(f"{sid} | story {story.label} не видна этому аккаунту (пропуск для сессии)")
    ...
    async def run_session(self, path):
        ...
        try:
            ...
            if not targets:
                ...
                return "no_targets"
            log.info(
                f"{sid} | 🎯 целей: {users_n} взаимных (100% круг) + {groups_n} групп | "
                f"режим: ЛС↔группы параллельно"
            )
    ...
    def _pick_next_target(
        ...
        """
        Параллельно ЛС + группы: строго по очереди, 100% контактов за круг.
        ЛС1 → группа1 → ЛС2 → группа2 → … без пропусков и ratio.
        """
        has_u = contact_idx < len(users)
        has_g = group_idx < len(groups)
        skip_ls = (
            config.SHADOW_SKIP_CONTACT_STREAK > 0
            and shadow_streak >= config.SHADOW_SKIP_CONTACT_STREAK
        )

        if not has_u and not has_g:
            return None, 0, 0, prefer_user

        if skip_ls and has_g:
            return groups[group_idx], contact_idx, group_idx + 1, prefer_user
        ...
        if not has_g:
            if skip_ls:
                return None, contact_idx, group_idx, prefer_user
            return users[contact_idx], contact_idx + 1, group_idx, prefer_user

        if skip_ls:
            return groups[group_idx], contact_idx, group_idx + 1, prefer_user

        if not config.PARALLEL_CONTACTS_GROUPS:
            if contact_idx < len(users):
                return users[contact_idx], contact_idx + 1, group_idx, prefer_user
            return groups[group_idx], contact_idx, group_idx + 1, prefer_user

        if prefer_user:
            return users[contact_idx], contact_idx + 1, group_idx, False
        return groups[group_idx], contact_idx, group_idx + 1, True

    async def send_loop(self, client, sid, path, targets, rng, story_cache, session_ready):
        errors = 0
        start = time.time()
        sent_local = 0
        sent_contacts_local = 0
        shadow_local = 0
        shadow_streak = 0
        circles_done = 0
        sess_file = self._cursor_key(path)
        stint_sec = config.REFRESH_INTERVAL
        hard_sec = self._stint_hard_sec()

        users = [t for t in targets if t.kind == "user"]
        groups = [t for t in targets if t.kind == "group"]
        contact_idx, group_idx, prefer_user = self._load_session_cursor(
            sess_file, len(users), len(groups)
        )
        if contact_idx > 0 or group_idx > 0:
            log.info(
                f"{sid} | ↩ продолжение круга: ЛС {contact_idx}/{len(users)}, "
                f"группы {group_idx}/{len(groups)}"
            )

        while True:
            elapsed = time.time() - start
            if elapsed >= hard_sec:
                self._save_session_cursor(sess_file, contact_idx, group_idx, prefer_user)
                log.warning(
                    f"{sid} | ♻️ hard-cap {int(hard_sec)}с, курсор "
                    f"ЛС {contact_idx}/{len(users)} группы {group_idx}/{len(groups)}"
                )
                return "rotate"
            if elapsed >= stint_sec:
                self._save_session_cursor(sess_file, contact_idx, group_idx, prefer_user)
                log.info(
                    f"{sid} | ♻️ смена слота ({int(stint_sec)}с), кругов: {circles_done}, курсор "
                    f"ЛС {contact_idx}/{len(users)} группы {group_idx}/{len(groups)} | "
                    f"реально {sent_local} (ЛС {sent_contacts_local}), тень {shadow_local}"
                )
                return "rotate"

            target, contact_idx, group_idx, prefer_user = self._pick_next_target(
                users, groups, contact_idx, group_idx, prefer_user, shadow_streak
            )
            if target is None:
                circles_done += 1
                contact_idx = 0
                group_idx = 0
                prefer_user = True
                left = max(0, int(stint_sec - (time.time() - start)))
                log.info(
                    f"{sid} | 🔁 круг #{circles_done}: все {len(users)} ЛС + {len(groups)} групп | "
                    f"реально {sent_local} (ЛС {sent_contacts_local}), тень {shadow_local} | "
                    f"осталось слота ~{left}с → новый круг"
                )
                await asyncio.sleep(jitter(config.DELAY_CYCLES, 0.0, rng, 5.0))
                continue

            kind_tag = "ЛС" if target.kind == "user" else "группа"
            pos = f"{contact_idx}/{len(users)}" if target.kind == "user" else f"{group_idx}/{len(groups)}"
            ...
            try:
                ...
                for _ in range(attempts):
                    ...
                    try:
                        ...
                        log.warning(
                            ...
                            f"({pos}) | {story.label} | серия: {shadow_streak}"
                        )
                ...
                if verified_ok and story is not None:
                    ...
                    sent_local += 1
                    if target.kind == "user":
                        sent_contacts_local += 1
                    shadow_streak = 0
                    errors = 0
                    log.info(
                        f"{sid} | ✅ реально → {kind_tag} {target.label} "
                        f"({pos}) | {story.label} | всего: {n}"
                    )
                    if target.kind == "user":
                    ...
                    base_delay = config.DELAY_MESSAGES
                    if target.kind == "user" and config.DELAY_MESSAGES_CONTACT > 0:
                        base_delay = config.DELAY_MESSAGES_CONTACT
                    await asyncio.sleep(
                        jitter(base_delay * delay_mul, 0.3, rng, max(0.15, base_delay * 0.25))
                    )
                    continue
                ...
                if target_bad:
                    if target.kind == "user" and contact_idx > 0:
                        users.pop(contact_idx - 1)
                        contact_idx = max(0, contact_idx - 1)
                    elif target.kind == "group" and group_idx > 0:
                        groups.pop(group_idx - 1)
                        group_idx = max(0, group_idx - 1)
                    if not users and not groups:
                        log.warning(f"{sid} | ⚠️ все цели битые — пересбор")
                        fresh = await collect_targets(client, rng, config.CONTACT_MAX_OFFLINE_DAYS)
                        users[:] = [t for t in fresh if t.kind == "user"]
                        groups[:] = [t for t in fresh if t.kind == "group"]
                        contact_idx = group_idx = 0
                ...
                log.warning(f"{sid} | ⚠ {target.label}: {humanize(ue)} — убираю из круга")
                if target.kind == "group" and group_idx > 0:
                    groups.pop(group_idx - 1)
                    group_idx = max(0, group_idx - 1)
                await asyncio.sleep(jitter(0.5, 0.2, rng, 0.3))
                continue
    ...
    async def main(self):
        ...
        log.info(
            f"💬 Старт (stories, ЛС↔группы 100%). Лимит сессий: {config.MAX_SESSIONS}, "
