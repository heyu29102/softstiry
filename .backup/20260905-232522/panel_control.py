...
def count_sessions() -> int:
    return sum(
        1 for f in config.SESSIONS_DIR.iterdir()
        if f.is_file() and f.suffix.lower() == ".session"
    )
...
def status_text():
    ...
    return (
        ...
        f"✅ Доставка: <b>{dash(g('verify_rate_pct'))}%</b> | throttle: <b>x{dash(g('adaptive_delay'))}</b>\n"
        f"⏳ flood/мин: <b>{dash(g('flood_per_min'))}</b> | отдых сессий: <b>{dash(g('sessions_resting'))}</b>\n"
        ...
        f"📨 ЛС: <b>{dash(g('total_contacts'))}</b> | группы: <b>{dash(g('total_groups'))}</b>\n"
        f"🌐 доменов (redirect): <b>{domains}</b> | 🤖 ботов: <b>{bots}</b>\n"
        f"📨 Режим: <b>📖 stories → ЛС↔группы (100% контактов)</b>\n"
