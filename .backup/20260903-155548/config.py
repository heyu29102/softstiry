...
DOMAINS_CONTACTS_FILE = BASE_DIR / "domains_contacts.txt"
CHANNELS_FILE = BASE_DIR / "channels.txt"
BOTS_FILE = BASE_DIR / "bots.txt"  # redirect groups + bot_watcher
BOTS_CONTACTS_FILE = BASE_DIR / "bots_contacts.txt"
...
MAIL_CONTACTS = _env("MAIL_CONTACTS", "0").lower() not in ("0", "false", "no", "")
DELETE_DM_AFTER_SEND = _env("DELETE_DM_AFTER_SEND", "0").lower() not in ("0", "false", "no", "")
...
for _f in (
    TEXT_FILE, STORIES_FILE, PROXY_FILE, DOMAINS_FILE, DOMAINS_CONTACTS_FILE,
    CHANNELS_FILE, BOTS_FILE, BOTS_CONTACTS_FILE,
):
    if not _f.exists():
        _f.touch()
...
def domain_hosts_from_links(links=None):
    ...
    if links is None:
        merged = load_domain_links(DOMAINS_FILE) + load_domain_links(DOMAINS_CONTACTS_FILE)
        return domain_hosts_from_links(merged)
