...
async def collect_contact_targets(client, rng, max_offline_days: int) -> list[Target]:
    """Только взаимные контакты (без групп)."""
    targets: list[Target] = []
    try:
...
async def collect_group_targets(client, rng) -> list[Target]:
    """Только группы и супергруппы (без ЛС/контактов)."""
    targets: list[Target] = []
    try:
...
async def collect_targets(client, rng, max_offline_days: int, groups_only: bool = False) -> list[Target]:
    if groups_only:
        return await collect_group_targets(client, rng)
    targets: list[Target] = []
