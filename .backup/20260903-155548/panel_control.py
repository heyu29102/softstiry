...
def status_text():
    ...
    domains_groups = len(config.load_domain_links(config.DOMAINS_FILE))
    bots_groups = len(config.load_bot_links(config.BOTS_FILE))
    mode_label = "только группы/супергруппы"
    return (
        f"🖥 app.py: {'🟢 запущен' if run else '🔴 остановлен'} {('PID ' + str(read_pid())) if run else ''}\n"
        f"⏱ Аптайм: <b>{up}</b>\n"
        f"📩 Реально доставлено: <b>{dash(g('total_sent'))}</b>\n"
        f"👻 Тень (API ok, в чате нет): <b>{dash(g('shadow_total'))}</b>\n"
        f"📈 Реально/мин: <b>{dash(g('sent_per_min'))}</b> | 👻 тень/мин: <b>{dash(g('shadow_per_min'))}</b>\n"
        f"✅ Доставка: <b>{dash(g('verify_rate_pct'))}%</b> | ⏳ flood/мин: <b>{dash(g('flood_per_min'))}</b>\n"
        f"🧵 Активных: <b>{dash(g('active_sessions'))}/{dash(g('max_sessions'))}</b> | в очереди: <b>{dash(g('pending'))}</b>\n"
        f"📂 sessions: <b>{count_sessions()}</b> | 📝 блоков: <b>{blocks}</b> | 🖼 фото: <b>{photos}</b>\n"
        f"🌐 домены (группы): <b>{domains_groups}</b> | 🤖 боты: <b>{bots_groups}</b>\n"
        f"📨 Режим: <b>🖼 фото+текст → {mode_label}</b>\n"
