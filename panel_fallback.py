...
def _share_quick_reply(text):
    lines = [ln.strip() for ln in text.splitlines() if ln.strip()]
    if not lines:
        return None
    if not all(normalize_share_link(ln) for ln in lines):
        return None
    if not all("share.google" in normalize_share_link(ln) for ln in lines):
        return None
    added = removed = 0
    ...
    for line in lines:
        ...
        if action == "added":
            ...
            removed += 1
    return (
        f"🔗 Share из сообщения: +{added} / −{removed}\n"
        f"В пуле: <b>{total}</b>"
    )
...
async def fallback(message):
    ...
    quick = _share_quick_reply(text)
    if quick:
        await message.answer(quick)
        return
    await message.answer(
        "Меню: /start\n"
        "Или кинь share.google ссылку — добавлю в пул."
    )
